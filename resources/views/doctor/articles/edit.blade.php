        ...
        <div class="w-full mb-8 overflow-hidden rounded-lg shadow-xs">
            <div class="w-full overflow-x-auto">
                ...
                <form
                    ...
                    action="{{ route('doctor.articles.update', $article) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <label class="block text-sm w-full md:w-1/2 md:pr-2">
                        ...
                        <input
                            ...
                            placeholder="Masukkan judul artikel" name="title" value="{{ $article->title }}" />
                    ...
                    <label class="flex flex-col text-sm w-full pb-3">
                        ...
                        <div class="h-[500px]">
                            <textarea class="form-control border-0" name="body" id="body">{{ $article->body }}</textarea>
                            <x-input-error :messages="$errors->get('body')" class="mt-2" />
                        </div>
                    </label>
                    <x-main-button type="submit" class="w-full">POST</x-main-button>
                </form>
            </div>
        </div>
    </div>
