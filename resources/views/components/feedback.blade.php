@if ($user && !$user->hasRatedWebsite())
    <div class="feedback fixed w-96 bottom-4 glassmorphism-box min-h-[200px]">
            ...
            <div class="flex flex-col">
                <div class="flex">
                    <div class="w-1/5">
                        <div class="h-12 w-12 rounded-full overflow-hidden bg-black">
                            @if ($user->isDoctor())
                                <img class="object-cover object-center w-full h-full" src="{{ $user->doctor && $user->doctor->photo ? $user->doctor->photo : '/assets/images/default.jpg' }}" alt="">
                            @else
                                <img class="object-cover object-center w-full h-full" src="{{ $user->client && $user->client->photo ? $user->client->photo : '/assets/images/default.jpg' }}" alt="">
                            @endif
                        </div>
                    </div>
                    <div class="flex flex-col pl-3 pr-2">
                        <h6 class="text-[18px] text-dark-gray">{{ $user->name }}</h6>
                        <p class="text-[14px] text-dark-gray">ulasan bersifat publik serta menyertakan info akun dan
                            perangkat anda</p>
                    </div>
                </div>
                ...
                <div class="relative mt-4">
                    <form id="formFeedback">
                        ...
                        <button type="submit"
                            class="absolute flex justify-center items-center top-[50%] translate-y-[-50%] right-2 h-7 w-7">
        ...
        <script>
            ...
            hideFeedbackIcon.addEventListener('click', hideFeedback);
        </script>
        ...
        <script>
            ...
            async function postFeedback() {
                const comment = formFeedback[0].value;
                const data = {
                    comment,
                    rating,
                    user_id: {{ $user->id }}
